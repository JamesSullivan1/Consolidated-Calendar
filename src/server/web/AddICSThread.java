...
/**
 * Provides threaded support for parsing an ics file located
 * at a given URL and integrating its events into the
...
public class AddICSThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			ArrayList<String> errors = (ArrayList<String>)session.getAttribute("parseErrors");
			synchronized (errors) {
				errors.add("Error parsing ICS calendar data from file: "+e.getMessage());
			}
			ThreadHelper.decrementThreadCount(session);
			return;
		}
		...
		try {
			...
			ArrayList<String> errors = (ArrayList<String>)session.getAttribute("parseErrors");
			synchronized (errors) {
				errors.add("Error parsing ICS event data from file: "+e.getMessage());
			}
			ThreadHelper.decrementThreadCount(session);
			return;
		}
